...
class Activation(object):
    ...
    def sigmoid(self, x):
        return 1.0 / (1.0 + np.exp(-self.params["sig_beta"] * x))
    ...
    def gelu_deriv(self, a):
        ...
        return 0.5 * (
            ...
            1.0
            - np.square(np.tanh(np.sqrt(2 / np.pi) * (x + 0.044715 * np.power(x, 3))))
        ) * np.sqrt(
            2 / np.pi
        ) * (
            1.0 + 0.134145 * np.square(x)
        )
    ...
    def __init__(
        self, activation: str, activation_params: Dict[str, float] = {}
    ) -> None:
        """
        ...
        self.params.setdefault("sig_beta", 1.0)  # sigmoid
        self.f_deriv = None
